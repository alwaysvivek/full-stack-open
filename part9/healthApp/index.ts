...
import computeMetabolicIndex from './bmiCalculator';
import analyzeTraining from './exerciseCalculator';
...
app.get('/bmi', (req, res) => {
  ...
  if (!weight || !height || isNotNumber(weight) || isNotNumber(height)) {
    ...
    const bmi = computeMetabolicIndex(Number(height), Number(weight));
...
app.post('/exercises', (req, res) => {
  ...
  const result = analyzeTraining(daily_exercises, target);
