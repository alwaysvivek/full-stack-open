...
export const toNewClient = (object: unknown): NewClient => {
  if (!object) {
    throw new Error("Missing client data");
  }
  if (typeof object !== "object") {
    throw new Error("Client data is in incorrect format. Client data must be an object");
  }
...
const parseMedicalConditionCodes = (object: unknown): Array<MedicalCondition["code"]> => {
  if (!object || typeof object !== "object" || !("diagnosisCodes" in object)) {
    return [] as Array<MedicalCondition["code"]>;
  }

  return object.diagnosisCodes as Array<MedicalCondition["code"]>;
};
...
export const toEntry = (object: unknown): EntryWithoutId => {
  ...
  const newEntry = {
    ...
    diagnosisCodes: parseMedicalConditionCodes(object),
  };
